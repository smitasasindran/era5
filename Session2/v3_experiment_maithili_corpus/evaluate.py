"""
Evaluate a trained v3 tokenizer: faithful-unit fertility per language,
computed over each language's full corpus (occurrence-level, matching v2 and
the instructor's own evaluate_tokenizer.py). Also computes the Hindi penalty
found in solution2/evaluate_tokenizer.py, which wasn't part of what we'd
been told about the grading rubric before discovering it there:
...
LANGS = ["en", "hi", "te", "mai"]
LANG_NAME = {"en": "English", "hi": "Hindi", "te": "Telugu", "mai": "Maithili"}
...
def evaluate(tokenizer_path):
    ...
    x1 = results["en"]["ratio"]
    x1_pass = x1 < 1.2
    hindi_penalty = math.exp(max(0.0, results["hi"]["ratio"] / 1.2 - 1.0))
    hindi_adjusted_score = score / hindi_penalty if score else None
    ...
    print(f"Hindi penalty factor = {hindi_penalty:.6f}   Hindi-adjusted score = {hindi_adjusted_score:.1f}")
    ...
    summary = {
        ...
        "hindi_adjusted_score": hindi_adjusted_score,
    }
...
if __name__ == "__main__":
    ...
    out_json = sys.argv[2] if len(sys.argv) > 2 else "../tokenizer-playground/models/v3/eval_results.json"
