...
Telugu/Telugu-script, Hindi+Maithili/Devanagari-joint) -- same strategy as
v1/v2's merge_tokenizers.py (safe because the 3 groups' scripts never
overlap -- see v1's docstring for why concatenating independently-trained
*same-script* tokenizers would corrupt merge order).
"""
import json
import sys
from tokenizers import Tokenizer, models, pre_tokenizers, decoders
from train_utils import train_single, himai_train, make_normalizer, UNK_TOKEN
...
if __name__ == "__main__":
    ...
    himai_tok = himai_train(quotas["himai"], quotas["hi_w"], quotas["mai_w"])

    merge_groups([en_tok, te_tok, himai_tok], out_path)
