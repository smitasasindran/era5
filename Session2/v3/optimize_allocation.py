...
Telugu, Hindi+Marathi-joint), same 4 languages, just on v3's much richer
HTML-based corpus. Binary search for the smallest common fertility target
rho such that the three groups' minimum-vocab-for-rho sums to <= 10,000.
...
tokenizers can't just be concatenated). The Hindi:Marathi weight was
re-tuned for this corpus (sweep_hi_mr_weight.py): 3:7 here vs v2's 2:3 --
Hindi's corpus grew proportionally more than Marathi's under the richer
fetch (ratio went from ~1.8x to ~3.0x), so Marathi needs more counter-weight
than before.
"""
